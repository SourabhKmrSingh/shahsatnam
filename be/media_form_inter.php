...
$youtube_url = $validation->input_validate($_POST['youtube_url']);
$youtube_video_id = $validation->input_validate($_POST['youtube_video_id']);
$title = $validation->input_validate($_POST['title']);
$title_id = $validation->input_validate($_POST['title_id']);

$btn_url = $validation->input_validate($_POST['btn_url']);
if($title_id == "")
{
	$title_id = $title;
}
...
$fields = array('title'=>$title, 'class'=> $class, 'subject'=> $subject, 'medium'=> $medium, 'youtube_url'=> $youtube_url, 'youtube_video_id'=> $youtube_video_id, 'btn_url'=> $btn_url, 'title_id'=>$title_id, 'order_custom'=>$order_custom, 'imgName'=>$imgName, 'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$mediaQueryResult = $db->insert("rb_media", $fields);
	if(!$mediaQueryResult)
	{
...
else if($mode == "edit")
{
	...
	$mediaQueryResult = $db->update("rb_media", $fields, array('mediaid'=>$mediaid));
	if(!$mediaQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: media_view.php$search_filter");
	exit();
}
?>