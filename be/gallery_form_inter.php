...
// $dupresult = $db->check_duplicates('rb_gallery', 'imageid', $imageid, 'title_id', strtolower($title_id), $mode);
// if($dupresult >= 1)
// {
...
if($mode == "insert")
{
	...
	$galleryQueryResult = $db->insert("rb_gallery", $fields);
...
else if($mode == "edit")
{
	...
	$galleryQueryResult = $db->update("rb_gallery", $fields, array('imageid'=>$imageid));
	if(!$galleryQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: gallery_view.php$search_filter");
	exit();
}
?>