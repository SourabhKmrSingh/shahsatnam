...
$dupresult = $db->check_duplicates('rb_news', 'newsid', $newsid, 'title_id', strtolower($title_id), $mode);
if($dupresult >= 1)
{
...
if($mode == "insert")
{
	...
	$categoryQueryResult = $db->insert("rb_news", $fields);
...
else if($mode == "edit")
{
	...
	$categoryQueryResult = $db->update("rb_news", $fields, array('newsid'=>$newsid));
	if(!$categoryQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: news_view.php$search_filter");
	exit();
}
?>