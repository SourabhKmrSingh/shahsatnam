...
<div class="form-rows-custom mt-3">
	...
	<div class="row mb-3">
		<div class="col-sm-3">
			<label for="imgName_side">Upload Side Image</label>
		</div>
		<div class="col-sm-9">
			<input type="file" name="imgName_side" id="imgName_side">
			<input type="hidden" name="old_imgName_side" id="old_imgName_side" value="<?php if($mode == 'edit') echo $validation->db_field_validate($pagesRow['imgName_side']); ?>" />
			<?php if($mode == 'edit' and $pagesRow['imgName_side'] != "") { ?>
				<div class="mt-2 links">
					<img src="<?php echo IMG_THUMB_LOC; echo $validation->db_field_validate($pagesRow['imgName_side']); ?>" title="<?php echo $validation->db_field_validate($pagesRow['imgName_side']); ?>" class="img-responsive mh-51" /><br>
					<a href="<?php echo IMG_MAIN_LOC; echo $validation->db_field_validate($pagesRow['imgName_side']); ?>" target="_blank">Click to Download</a> | <a href="page_form.php?mode=edit&pageid=<?php echo $pageid; ?>&q=imgdel_side" onClick="return del();">Delete</a>
				</div>
			<?php } ?>
			<em class="d-block mt-1">File should be Image and size under <?php echo $validation->convertToReadableSize($configRow['image_maxsize']); ?><br>Image extension should be .jpg, .jpeg, .png, .gif</em>
		</div>
		...
		<div class="col-sm-12">
			...
			?>
				...
				<?php if($_SESSION['per_delete'] == "1") { ?>
					<a HREF="page_actions.php?q=del&pageid=<?php echo $pagesRow['pageid']; ?>" class="btn  btn-sm btn_delete" onClick="return del();"><i class="fas fa-trash"></i>&nbsp;&nbsp;Delete</a>
				<?php } ?>
			<?php
			}
			?>
