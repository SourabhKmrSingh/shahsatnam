...
<div CLASS="row">
	<div CLASS="col-lg-12">
		<h1 CLASS="page-header">Enquiries <?php if($_SESSION['per_write'] == "1") { ?><a href="enquiry_form.php?mode=insert"  class="btn mb_inline btn-sm btn_submit ml-3 mt-1">Add New</a><?php } ?></h1>
	</div>
</div>
...
<table class="table table-striped table-view" cellspacing="0" width="100%">
	...
	if($data['num_rows'] >= 1)
	{
		...
		<tr class="text-center has-row-actions">
			...
			<td data-label="Name - ">
				<a href="enquiry_form.php?mode=edit&enquiryid=<?php echo $validation->db_field_validate($enquiryRow['enquiryid']); ?>" class="fw-500"><?php echo $validation->db_field_validate($enquiryRow['first_name']); ?></a>
			...
			<td data-label="Status - "><font color="<?php if($enquiryRow['status'] == "active") { echo "green"; } else { echo "red"; } ?>"><?php echo $validation->db_field_validate(ucfirst($enquiryRow['status'])); ?></font></td>
