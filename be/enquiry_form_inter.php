...
$courseid = $validation->input_validate($_POST['courseid']);
$registration_date = $validation->input_validate($_POST['registration_date']);
$status = $validation->input_validate($_POST['status']);
$room_type = $validation->input_validate($_POST['room_type']);
$check_in = $validation->input_validate($_POST['check_in']);
$check_out = $validation->input_validate($_POST['check_out']);
$enquiry_type = $validation->input_validate($_POST['enquiry_type']);
...
$fields = array( 'first_name'=>$first_name, 'last_name'=>$last_name, 'email'=>$email, 'mobile'=>$mobile, 'subject'=>$subject, 'courseid'=> $courseid, 'registration_date'=> $registration_date, 'message'=>$message, 'remarks'=>$remarks, 'status'=>$status, 'user_ip'=>$user_ip);
...
else if($mode == "edit")
{
	...
	$enquiryQueryResult = $db->update("rb_enquiries", $fields, array('enquiryid'=>$enquiryid));
	if(!$enquiryQueryResult)
	{
		echo mysqli_error($connect);
		exit();
