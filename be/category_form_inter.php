...
$brandid = $validation->input_validate($_POST['brandid']);
...
$dupresult = $db->check_duplicates('rb_categories', 'categoryid', $categoryid, 'title_id', strtolower($title_id), $mode);
if($dupresult >= 1)
{
...
$fields = array('brandid'=>$brandid ,'title'=>$title, 'title_id'=>$title_id, 'tagline'=>$tagline, 'order_custom'=>$order_custom, 'url'=>$url, 'url_target'=>$url_target, 'meta_title'=>$meta_title, 'meta_keywords'=>$meta_keywords, 'meta_description'=>$meta_description, 'description'=>$description, 'imgName'=>$imgName, 'fileName'=>$fileName, 'home_priority'=>$home_priority, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$categoryQueryResult = $db->insert("rb_categories", $fields);
...
else if($mode == "edit")
{
	...
	$categoryQueryResult = $db->update("rb_categories", $fields, array('categoryid'=>$categoryid));
	if(!$categoryQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: category_view.php$search_filter");
	exit();
}
?>