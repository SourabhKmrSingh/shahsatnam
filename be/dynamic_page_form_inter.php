...
$dupresult = $db->check_duplicates('rb_dynamic_pages', 'pageid', $pageid, 'title_id', strtolower($title_id), $mode);
if($dupresult >= 1)
{
...
if($iconTName != "")
{
	...
	if($mode == "edit")
	{
		$delresult = $media->filedeletion('rb_dynamic_pages', 'pageid', $pageid, 'icon', IMG_MAIN_LOC, IMG_THUMB_LOC);
	}
}
...
if($mode == "insert")
{
	...
	$pageQueryResult = $db->insert("rb_dynamic_pages", $fields);
	if(!$pageQueryResult)
	{
...
else if($mode == "edit")
{
	...
	$pageQueryResult = $db->update("rb_dynamic_pages", $fields, array('pageid'=>$pageid));
	if(!$pageQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: dynamic_page_view.php$search_filter");
	exit();
}
?>