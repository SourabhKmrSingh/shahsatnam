...
$main_menu = $validation->input_validate($_POST['main_menu']);
if($main_menu=='')
{
	$main_menu = 0;
}
$sub_menu = $validation->input_validate($_POST['sub_menu']);
if($sub_menu=='')
{
	$sub_menu = 0;
}
...
$old_imgName_side = $validation->input_validate($_POST['old_imgName_side']);
...
$dupresult = $db->check_duplicates('rb_pages', 'pageid', $pageid, 'title_id', strtolower($title_id), $mode);
if($dupresult >= 1)
{
...
$imgTName_side = $_FILES['imgName_side']['name'];
if($imgTName_side != "")
{
	$handle = new Upload($_FILES['imgName_side']);
    if($handle->uploaded)
	{
		$handle->file_force_extension = true;
		$handle->file_max_size = $validation->db_field_validate($configRow['image_maxsize']);
		...
		if($handle->processed)
		{
			$imgName_side = $handle->file_dst_name;
		}
		else
		{
			$_SESSION['error_msg'] = $handle->error.'!';
	...
	if($mode == "edit")
	{
		$delresult = $media->filedeletion('rb_pages', 'pageid', $pageid, 'imgName_side', IMG_MAIN_LOC, IMG_THUMB_LOC);
	}
}
...
$fields = array('main_menu'=>$main_menu, 'sub_menu'=>$sub_menu, 'title'=>$title, 'title_id'=>$title_id, 'tagline'=>$tagline, 'order_custom'=>$order_custom, 'url'=>$url, 'url_target'=>$url_target, 'meta_title'=>$meta_title, 'meta_keywords'=>$meta_keywords, 'meta_description'=>$meta_description, 'description'=>$description, 'imgName'=>$imgName, 'imgName_side'=>$imgName_side, 'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$pagesQueryResult = $db->insert("rb_pages", $fields);
	if(!$pagesQueryResult)
	{
...
else if($mode == "edit")
{
	...
	$pagesQueryResult = $db->update("rb_pages", $fields, array('pageid'=>$pageid));
	if(!$pagesQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: page_view.php$search_filter");
	exit();
}
?>