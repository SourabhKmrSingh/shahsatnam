...
<div CLASS="row">
	<div CLASS="col-lg-12">
		<h1 CLASS="page-header">Categories <?php if($_SESSION['per_write'] == "1") { ?><a href="category_form.php?mode=insert"  class="btn mb_inline btn-sm btn_submit ml-3 mt-1">Add New</a><?php } ?></h1>
	</div>
</div>

<form name="form_actions" method="POST" action="category_actions.php" ENCTYPE="MULTIPART/FORM-DATA">
<div class="row">
	<div class="col-sm-12 mb-0">
		<div class="form-inline">
			...
			<a href="category_view.php" class="btn  mb_inline btn-sm btn_delete ml-sm-2 ml-md-0 mb-2">Clear</a>
		</div>
	</div>
</div>
...
<table class="table table-striped table-view" cellspacing="0" width="100%">
	...
	<tr>
		...
		<th class="<?php echo $th_sort1." ".$th_order_cls1; ?>"><a href="category_view.php?orderby=title&order=<?php echo $th_order1; echo $url_parameters; ?>"><span>Title</span> <span class="sorting-indicator"></span></a></th>
		<th>Author</th>
		<th>Brand / Solution</th>
		<th class="<?php echo $th_sort2." ".$th_order_cls2; ?>"><a href="category_view.php?orderby=order_custom&order=<?php echo $th_order2; echo $url_parameters; ?>"><span>Order</span> <span class="sorting-indicator"></span></a></th>
		<th>Title ID</th>
		<th>Status</th>
		<th class="<?php echo $th_sort3." ".$th_order_cls3; ?>"><a href="category_view.php?orderby=createdate&order=<?php echo $th_order3.''.$url_parameters; ?>"><span>Date</span> <span class="sorting-indicator"></span></a></th>
	</tr>
	</thead>
	<tbody>
	<?php
	if($data['num_rows'] >= 1)
	{
		...
		<tr class="text-center has-row-actions">
			<td class="text-center" data-label=""><input type="checkbox" name="del_items[]" value="<?php echo $validation->db_field_validate($categoryRow['categoryid']); ?>"/></td>
			<td data-label="Title - ">
				<a href="category_form.php?mode=edit&categoryid=<?php echo $validation->db_field_validate($categoryRow['categoryid']); ?>" class="fw-500"><?php echo $validation->db_field_validate($categoryRow['title']); ?></a>
				
				<div class="row row-actions">
					<div class="col-sm-12">
						<?php if($_SESSION['per_update'] == "1") { ?>
							<a href="category_form.php?mode=edit&categoryid=<?php echo $validation->db_field_validate($categoryRow['categoryid']); ?>">Edit</a>
							 | 
						<?php } ?>
						<?php if($_SESSION['per_delete'] == "1") { ?>
							<a href="category_actions.php?q=del&categoryid=<?php echo $validation->db_field_validate($categoryRow['categoryid']); ?>" onClick="return del();" class="delete">Delete</a>
							 | 
						<?php } ?>
						
						<a href="product_view.php?categoryid=<?php echo $validation->db_field_validate($categoryRow['categoryid']); ?>">Product(s)</a>
					</div>
				</div>
			</td>
			<td data-label="Author - "><a href="category_view.php?userid=<?php echo $userid; ?>"><?php echo $validation->db_field_validate($userRow['display_name']); ?></a></td>
			<td data-label="Brand / Solution - "><a href="category_view.php?mode=edit&brandid=<?php echo $validation->db_field_validate($categoryRow['brandid']); ?>" class="fw-500"><?php echo $validation->db_field_validate($brandRow['title']); ?></a></td>
