...
$stat_title_first = $validation->input_validate($_POST['stat_title_first']);
$stat_value_first = $validation->input_validate($_POST['stat_value_first']);
$stat_title_second = $validation->input_validate($_POST['stat_title_second']);
$stat_value_second = $validation->input_validate($_POST['stat_value_second']);
$stat_title_third = $validation->input_validate($_POST['stat_title_third']);
$stat_value_third = $validation->input_validate($_POST['stat_value_third']);
...
// $dupresult = $db->check_duplicates('rb_stats', 'statsid', $statsid, 'title_id', strtolower($title_id), $mode);
// if($dupresult >= 1)
// {
...
if($imgName == "")
{
	...
	$fields = array('title'=>$title, 'title_id'=>$title_id, 'tagline'=>$tagline, 'order_custom'=>$order_custom, 'url'=>$url, 'url_target'=>$url_target, "stat_title_first" => $stat_title_first, "stat_value_first"=>$stat_value_first,"stat_title_second"=>$stat_title_second, 'stat_value_second'=>$stat_value_second,'stat_title_third'=>$stat_title_third,'stat_value_third'=>$stat_value_third, 'description'=>$description, 'imgName'=>$imgName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$statsQueryResult = $db->insert("rb_stats", $fields);
	if(!$statsQueryResult)
	{
...
else if($mode == "edit")
{
	...
	$statsQueryResult = $db->update("rb_stats", $fields, array('statsid'=>$statsid));
	if(!$statsQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: stats_view.php$search_filter");
	exit();
}
?>