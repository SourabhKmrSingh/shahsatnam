...
// $dupresult = $db->check_duplicates('rb_sliders', 'sliderid', $sliderid, 'title_id', strtolower($title_id), $mode);
// if($dupresult >= 1)
// {
...
if($mode == "insert")
{
	...
	$sliderQueryResult = $db->insert("rb_sliders", $fields);
...
else if($mode == "edit")
{
	...
	$sliderQueryResult = $db->update("rb_sliders", $fields, array('sliderid'=>$sliderid));
	if(!$sliderQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: slider_view.php$search_filter");
	exit();
}
?>