...
if($mode == "edit")
{
	if(isset($_GET['coursehighlightid']))
	{
...
$dupresult = $db->check_duplicates('rb_course_highlight', 'coursehighlightid', $coursehighlightid, 'title_id', strtolower($title_id), $mode);
if($dupresult >= 1)
{
...
if($iconTName != "")
{
	...
	if($mode == "edit")
	{
		$delresult = $media->filedeletion('rb_course_highlight', 'coursehighlightid', $coursehighlightid, 'icon', IMG_MAIN_LOC, IMG_THUMB_LOC);
	}
}
...
$fields = array('productid'=> $productid, 'title'=>$title, 'title_id'=>$title_id, 'order_custom'=>$order_custom,  'description'=>$description, 'imgName'=>$imgName,  'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$pageQueryResult = $db->insert("rb_course_highlight", $fields);
	if(!$pageQueryResult)
	{
...
else if($mode == "edit")
{
	...
	$pageQueryResult = $db->update("rb_course_highlight", $fields, array('coursehighlightid'=>$coursehighlightid));
	if(!$pageQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: course_highlight_view.php$search_filter");
	exit();
}
?>