...
<div CLASS="row">
	<div CLASS="col-lg-12">
		<h1 CLASS="page-header">Stats Section <?php if($_SESSION['per_write'] == "1") { ?><a href="stats_form.php?mode=insert"  class="btn mb_inline btn-sm btn_submit ml-3 mt-1">Add New</a><?php } ?></h1>
	</div>
</div>

<form name="form_actions" method="POST" action="stats_actions.php" ENCTYPE="MULTIPART/FORM-DATA">
<div class="row">
	<div class="col-sm-12 mb-0">
		<div class="form-inline">
			...
			<a href="stats_view.php" class="btn  mb_inline btn-sm btn_delete ml-sm-2 ml-md-0 mb-2">Clear</a>
		</div>
	</div>
</div>
...
<table class="table table-striped table-view" cellspacing="0" width="100%">
	...
	<tr>
		...
		<th class="<?php echo $th_sort1." ".$th_order_cls1; ?>"><a href="stats_view.php?orderby=title&order=<?php echo $th_order1; echo $url_parameters; ?>"><span>Title</span> <span class="sorting-indicator"></span></a></th>
		<th>Author</th>
		<th class="<?php echo $th_sort2." ".$th_order_cls2; ?>"><a href="stats_view.php?orderby=order_custom&order=<?php echo $th_order2; echo $url_parameters; ?>"><span>Order</span> <span class="sorting-indicator"></span></a></th>
		<th>Status</th>
		<th class="<?php echo $th_sort3." ".$th_order_cls3; ?>"><a href="stats_view.php?orderby=createdate&order=<?php echo $th_order3.''.$url_parameters; ?>"><span>Date</span> <span class="sorting-indicator"></span></a></th>
	</tr>
	</thead>
	<tbody>
	<?php
	if($data['num_rows'] >= 1)
	{
		foreach($data['result'] as $statsRow)
		{
		...
		<tr class="text-center has-row-actions">
			<td class="text-center" data-label=""><input type="checkbox" name="del_items[]" value="<?php echo $validation->db_field_validate($statsRow['statsid']); ?>"/></td>
			<td data-label="Title - ">
				<a href="stats_form.php?mode=edit&statsid=<?php echo $validation->db_field_validate($statsRow['statsid']); ?>" class="fw-500"><?php echo $validation->db_field_validate($statsRow['title']); ?></a>
				
				<div class="row row-actions">
					<div class="col-sm-12">
						<?php if($_SESSION['per_update'] == "1") { ?>
							<a href="stats_form.php?mode=edit&statsid=<?php echo $validation->db_field_validate($statsRow['statsid']); ?>">Edit</a>
							 | 
						<?php } ?>
						<?php if($_SESSION['per_delete'] == "1") { ?>
							<a href="stats_actions.php?q=del&statsid=<?php echo $validation->db_field_validate($statsRow['statsid']); ?>" onClick="return del();" class="delete">Delete</a>
						<?php } ?>
					</div>
				</div>
			</td>
			<td data-label="Author - "><a href="stats_view.php?userid=<?php echo $userid; ?>"><?php echo $validation->db_field_validate($userRow['display_name']); ?></a></td>
			<td data-label="Order - "><?php echo $validation->db_field_validate($statsRow['order_custom']); ?></td>
			<td data-label="Status - "><font color="<?php if($statsRow['status'] == "active") { echo "green"; } else { echo "red"; } ?>"><?php echo $validation->db_field_validate(ucfirst($statsRow['status'])); ?></font></td>
			<td class="date" data-label="Date - "><?php echo $validation->date_format_custom($statsRow['createdate']); ?> <br class="mb-hidden" />(<?php echo $validation->timecount("{$statsRow['createdate']} {$statsRow['createtime']}"); ?>)</td>
		</tr>
		<?php
		}
	}
